import { ACommandArgs, Stride_Color, TileType, UnitDirection, UnitFlags, VisualEffectConfig } from "library/game-logic/horde-types";
import { IProduceSpell } from "../IProduceSpell";
import { IUnitCaster } from "../IUnitCaster";
import { UnitProducerProfessionParams, UnitProfession } from "library/game-logic/unit-professions";
import { HordeColor, createHordeColor, createResourcesAmount } from "library/common/primitives";
...
export class Spell_dead_army extends IProduceSpell {
    protected static _ChargesReloadTime             : number = 60*50;
    ...
    protected static _MaxLevel                      : number = 1;
    protected static _DurationPerLevel       : Array<number> = [50].map(sec => sec * 50);
    protected static _ChargesCountPerLevel   : Array<number> = [1];
    protected static _NamePrefix                    : string = "Армия мёртвых";
    protected static _DescriptionTemplate           : string
        = "Призвать армию мертвецов вне леса. Время действия {0} сек.";
    ...
    private static _EffectConfig : VisualEffectConfig = HordeContentApi.GetVisualEffectConfig("#VisualEffectConfig_MagicCircle");
    ...
    constructor(caster: IUnitCaster, ...spellArgs: any[]) {
        ...
        var producerParams = caster.hordeUnit.Cfg.GetProfessionParams(UnitProducerProfessionParams, UnitProfession.UnitProducer);
        // @ts-ignore
        var produceList    = producerParams.CanProduceList;
        produceList.Clear();

        Spell_dead_army._DeadUnitTypes.forEach(unit => {
            produceList.Add(unit.GetHordeConfig());
        });
    ...
    protected _OnEveryTickActivated(gameTickNum: number): boolean {
        ...
        if (this._activatedTick + Spell_dead_army._DurationPerLevel[this.level] <= gameTickNum) {
            var returnedSouls = 0;
